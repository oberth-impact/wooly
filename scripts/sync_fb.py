...
def prune_orphan_images(posts):
    # type: (List[Dict]) -> None
    """Delete downloaded images no longer referenced by the current posts.

    Facebook CDN URLs are signed and rotate, so image_filename() produces a
    new name for the same photo on nearly every sync. Without pruning, the
    images directory grows without bound and eventually blows past the
    GitHub Pages 1 GB artifact limit, breaking deployment.
    """
    keep = set()
    for post in posts:
        local = post.get('_local_image')
        if local:
            keep.add(os.path.basename(local))
    ...
    for fname in os.listdir(IMAGES_DIR):
        if not fname.startswith('fb_'):
            continue  # never touch non-synced assets
        if fname in keep:
            continue
        try:
            ...
            print("Error removing {}: {}".format(fname, e))
    print("Pruned {} orphaned images ({} kept)".format(removed, len(keep)))
...
def main():
    ...
    # Drop images no longer referenced by the current post set so the
    # published site stays well under the GitHub Pages 1 GB artifact limit.
    prune_orphan_images(posts)
